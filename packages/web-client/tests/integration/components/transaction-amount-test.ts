...
module('Integration | Component | transaction-amount', function (hooks) {
  ...
  test('It disables the unlock button when amount entered is more than balance (18-decimal floating point)', async function (assert) {
    ...
    await fillIn('[data-test-transaction-amount-input]', daiToSend);
    assert.dom('[data-test-unlock-button]').isDisabled();
    await fillIn('[data-test-transaction-amount-input]', startDaiAmountString);
  ...
  test('It does not accept invalid values and ignores invalid characters typed in', async function (assert) {
    ...
    assert.dom('[data-test-transaction-amount-input]').hasValue('');
    assert.dom('[data-test-unlock-button]').isDisabled();

    await fillIn('[data-test-transaction-amount-input]', invalidDai1);
    assert.dom('[data-test-transaction-amount-input]').hasValue(invalidDai1);
    assert.dom('[data-test-unlock-button]').isDisabled();

    await fillIn('[data-test-transaction-amount-input]', validAmount);
    assert.dom('[data-test-transaction-amount-input]').hasValue(validAmount);
    assert.dom('[data-test-unlock-button]').isNotDisabled();

    await fillIn('[data-test-transaction-amount-input]', invalidDai2);
    assert.dom('[data-test-transaction-amount-input]').hasValue(validAmount);
    assert.dom('[data-test-unlock-button]').isNotDisabled();

    await fillIn('[data-test-transaction-amount-input]', '');
    assert.dom('[data-test-transaction-amount-input]').hasValue('');
    assert.dom('[data-test-unlock-button]').isDisabled();

    await typeIn('[data-test-transaction-amount-input]', invalidDai2);
    assert.dom('[data-test-transaction-amount-input]').hasValue('11');
    assert.dom('[data-test-unlock-button]').isNotDisabled();

    await fillIn('[data-test-transaction-amount-input]', '');
    await typeIn('[data-test-transaction-amount-input]', invalidDai3);
    assert.dom('[data-test-transaction-amount-input]').hasValue('1');
    assert.dom('[data-test-unlock-button]').isNotDisabled();

    await fillIn('[data-test-transaction-amount-input]', '');
    await typeIn('[data-test-transaction-amount-input]', invalidDai4);
    assert.dom('[data-test-transaction-amount-input]').hasValue('12');
    assert.dom('[data-test-unlock-button]').isNotDisabled();

    await fillIn('[data-test-transaction-amount-input]', startDaiString);
    assert.dom('[data-test-transaction-amount-input]').hasValue(startDaiString);
    assert.dom('[data-test-unlock-button]').isNotDisabled();
  });
});
